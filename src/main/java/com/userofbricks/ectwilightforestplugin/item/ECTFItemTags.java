...
public class ECTFItemTags {
    ...
    public static void loadTags() {
        REGISTRATE.get().addDataGenerator(ProviderType.ITEM_TAGS, tagsProvider -> {
            ...
            tagsProvider.addTag(bindForgeStorageBlock(TwilightForestPlugin.FIERY.getLocationName())).add(BlockItem.byBlock(TFBlocks.FIERY_BLOCK.get()));
            tagsProvider.addTag(bindForgeStorageBlock(TwilightForestPlugin.IRONWOOD.getLocationName())).add(BlockItem.byBlock(TFBlocks.IRONWOOD_BLOCK.get()));
            tagsProvider.addTag(bindForgeStorageBlock(TwilightForestPlugin.STEELEAF.getLocationName())).add(BlockItem.byBlock(TFBlocks.STEELEAF_BLOCK.get()));
            tagsProvider.addTag(bindForgeStorageBlock(TwilightForestPlugin.KNIGHTMETAL.getLocationName())).add(BlockItem.byBlock(TFBlocks.KNIGHTMETAL_BLOCK.get()));
        });
    }
}
